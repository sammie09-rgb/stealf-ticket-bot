// index.js - patched ticket bot (based on your backup, small safe upgrades)
// improvements:
// - safe thumbnail handling (avoids "expected to match a URL" crash)
// - ephemeral confirmation for panel command (sends the panel into channel but confirms to user ephemerally)
// - keeps the close/delete confirm flows and all original behavior intact
// - minor wording + color tweaks for a sleeker panel
...
function normalizeThumbUrl(raw) {
  if (!raw) return '';
  let v = raw.trim();
  // some services give protocol-relative urls like //cdn.domain/xxx
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isChatInputCommand()) {
      if (interaction.commandName === 'ticketpanel') {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            "welcome to the stealthy frog’s ticketing channel. if you have any questions or technical issues, click **open ticket** to contact a member of staff.\n\n" +
            "for partnership inquiries, click **partnership**.\n\n" +
    ...
    // open ticket group (support / partnerships)
    if (customId === 'open_support' || customId === 'open_partnerships') {
      ...
      // build permission overwrites carefully
      let category = null;
